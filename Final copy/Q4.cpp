...
public:
    ...
    void get_top_similarities(vector<pair<float, int> >& sim_indices, int num_top = 10) const {
        sim_indices.resize(S.size());
        for (int i = 0; i < S.size(); i++) {
            sim_indices[i] = make_pair(S[i], i);
        }
        sort(sim_indices.rbegin(), sim_indices.rend());
        sim_indices.resize(num_top);
    }


      void sort_sim(const vector<pair<float, int> >& sim_indices) {
        sort_indices.resize(sim_indices.size());
        for (int i = 0; i < sim_indices.size(); i++) {
            sort_indices[i] = sim_indices[i].second;
        }
    }
...
class Euclid : public BaseCadd_V2 {
public:
    vector<float> D;
