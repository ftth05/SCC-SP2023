...
public:
    ...
    void sim(std::vector<float>& D, int n_rows, int n_cols) {
    int n = sqrt(D.size()); // assuming D is a square matrix
    // the number of columns in matrix D.
    //int n = D.size[0].size();
    num_features = n_cols;
    S.resize(n_rows*n_rows);
    float gamma = 1.0 / n_rows;
...
public:
    ...
    void dist(std::vector<float>& X, int n_rows, int n_cols) {
        int n = sqrt(n_rows);
        num_features = n_cols;

        D.resize(n_rows*n_rows);
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                float sum = 0;
        ...
        // cout << "Euclid distance computed" << endl;
    }
...
public:
    ...
    void dist(std::vector<float>& X, int n_rows, int n_cols) {
        ...
        // cout << "Minkowski distance computed" << endl;
    }
};
...
int main() {
    ...
    while (std::getline(in_file, line)) {
        n_rows++;
        n_cols = std::count(line.begin(), line.end(), ' ') + 1;
        }
    ...
    cout <<"  known_drug "<< known_drug.size() << endl;
    ...
    for (int i = 0; i < n_rows; i++) {
        // MK1.dist(A[i], n_rows, n_cols);
        
    std::vector<float> row_vec(A[i], A[i] + n_cols);
    MK1.dist(row_vec, known_drug.data(), n_cols);
    }

    // Compute the distance vector D using MK2
    // for (int i = 0; i < n_rows; i++) {
    // MK2.dist(A[i], n_rows, n_cols);
    // std::vector<float> row_vec(A[i], A[i] + n_cols);
    // MK2.dist(row_vec, n_rows, n_cols);
    // }

    cout<<  MK1.S.size()   <<endl;
    cout<<  MK2.S.size()   <<endl;
    ...
    cerr << "Error: could not open result1.txt for writing" << endl;
    return 1;
    }
